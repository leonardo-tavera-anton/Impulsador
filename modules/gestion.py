...
def render(df):
    ...
    st.markdown(f"""
        <div style="background: linear-gradient(90deg, #1e3a8a, #3b82f6); padding:15px; border-radius:10px; color:white; margin-bottom:15px;">
            ...
            <p style='margin:0; opacity:0.8;'>Nuevo Chimbote | Guardado Automático y Multianual</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with f1:
        busqueda = st.text_input("🔍 Buscar por Nombre, DNI o Celular...", placeholder="Ej: Leonardo...")
    with f2:
    ...
    def handle_autosave():
        # Accedemos a los cambios detectados en el editor
        key_editor = f"editor_v14_{ano_activo}"
        if key_editor in st.session_state:
            ...
            if state["edited_rows"]:
                ...
                for row_idx, changes in state["edited_rows"].items():
                    # Obtener datos actuales de la fila que se está editando
                    actual_row = df_display.iloc[row_idx]
                    dni_v = actual_row['dni']
                    
                    # Traer el historial completo original para no borrar otros años
                    historial_total = df[df['dni'] == dni_v].iloc[0]['historial']
                    if not isinstance(historial_total, dict): historial_total = {}

                    # Obtener la lista de meses de la UI (o la actual si no cambió esa columna)
                    lista_ui = changes.get('Historial', actual_row['Historial'])
                    
                    # Convertir etiquetas (Ene/n.Ene) a valores (1/0)
                    dic_ano = {}
