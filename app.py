...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab-list"] { 
        gap: 15px; background-color: #0d1117; padding: 10px; border-radius: 10px; 
    }
    .stTabs [data-baseweb="tab"] { 
        height: 45px; background-color: #161b22; border-radius: 5px; 
        color: white; border: none; padding: 0px 20px;
    }
    .stTabs [aria-selected="true"] { 
        background-color: #58a6ff !important; color: black !important; font-weight: bold;
    }
    .stTextInput input { background-color: #0d1117; color: white; border: 1px solid #30363d; }
    </style>
""", unsafe_allow_html=True)

# 2. CARGA DE DATOS CENTRALIZADA (OPTIMIZADA PARA 47,000 REGISTROS)
@st.cache_data(ttl=3600, show_spinner=False)
def load_data():
    # Placeholder para avisar al usuario del progreso real
    msg_carga = st.empty()
    try:
        all_rows = []
        limit = 1000
        ...
        while True:
            msg_carga.info(f"⏳ Sincronizando base de datos... ({offset:,} registros descargados)")
        ...
        if not df.empty:
            # Limpieza masiva (se hace una sola vez para las 47k filas)
        ...
        msg_carga.empty() # Borra el mensaje de carga cuando termina
        return df
    except Exception as e:
        msg_carga.error(f"❌ Error crítico en carga de datos: {e}")
        return pd.DataFrame()
...
if not st.session_state.auth:
    ...
    # Carga la data (AQUÍ ES DONDE OCURRE LA MAGIA DEL CACHÉ)
    df_main = load_data()
